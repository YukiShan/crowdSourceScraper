...
def get_ranks(app_id, country_name, rank_list_name, app_genre_name=None):
    '''
    Get App Ranks by specified id, country names, list names and genres
    '''
    import getpass
    username = getpass.getuser()
    print 'Working ... Be patient, Master %s!' % username

    if country_name in app_countries:
        country_id = app_countries[country_name]
    else:
    ...
    rank_map = get_app_rank(app_id, country_id, rank_list, app_genre_code)
...
def main():
    get_ranks(['934596429', '1124271995'],'United States','Top Grossing','Games')
